...
%% User Options
num_prx = 2; % =2 or =1, specifies which version of model to use, and available associated user settings
if num_prx == 1
    v_sup = 10^(-5); % H2O2 supply rate; default: 10^(-5); (M/sec) (converted from M/sec) ranges 10^-7 M/sec to 10^-3 M/sec in Fig 3, based on varying levels of extracellular H202
elseif num_prx == 2
    bolus = 1; % µM (reasonable range is roughly >0 to 2000µM)

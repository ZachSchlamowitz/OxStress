...
system) and the dynamics of related transcription factor activity. 
...
To see the 1-prx and 2-prx ODE equations that are passed into these solvers, please refer to one of the
following sources: 
- for 1-prx equations: see the text of Selvaggio et al. (2018)
- for 2-prx equations: see either the text of Schlamowitz (2023) thesis document 
the thesis document 

However, the way MATLAB designed these ODE solvers (e.g., ode23s) is such that when calling them, one 
has to pass in a function handle (e.g., "@myfun") that refers to a separate script file. This separate
...
Zach Schlamowitz at zschlamowitz at gmail dot com). We make a parameters struct, Params, which we declare 
as a global variable; this way, parameters do not have to be passed in to the equations script diectly. 
...
it. This is because the loop in < plot_phenotypes > iterates over values of v_sup. The < selvaggio_model > parameters
...
vs v_sup. These are Fig.4a-c of Schlamowitz (2023).

Running < plot_ptrs > with num_prx = 2 will generate (up to) fifteen figures:
(1) replication figure Fig.4D of Schlamowitz (2023)
(2-3) prx hyperoxidation vs dose and time heatmaps, Fig.5 of Schlamowitz (2023)
(4) prx hyperoxidation vs dose only, Fig.6 of Schlamowitz (2023)
...
section as you like, set up desired plots in the "Plotting" section at the bottom of the file and hit run! This 
script is largely designed for playful exploration and experimentation, so the included plots are very minimal.
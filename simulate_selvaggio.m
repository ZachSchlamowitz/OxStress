...
% SIMULATE_SELVAGGIO
% Parameters:
% --> bolus: extracellular bolus of H2O2 (uM)
% --> intra_h2o2: initial value for intracellular H2O2 (uM)
% --> v_sup: H2O2 supply rate (uM/sec)
% --> cell_type: specify cell type as either 'MCF7  ' or 'HEK293'
% --> num_prx: specify number of peroxiredoxin species (=1 or =2) to toggle model version
% Arguments:
% <-- trajectories_raw: matrix containing discrete timecourse of PTRS states' raw values versus time
% <-- trajectories_frac: matrix containing discrete timecourse of PTRS states' values as fractions of total versus time
% Other:
% - called by < plot_ptrs.m >

%% Model
function [trajectories_raw, trajectories_frac] = ...
    simulate_selvaggio(bolus, intra_h2o2, v_sup, cell_type, num_prx)
% Initialize parameter struct and begin setup
global Params % global variable is used because cannot pass param struct into ode solver directly
Params = struct();
if isnan(v_sup) % set v_sup value based on function parameter 
    Params.v_sup = 10^(-5+6); % default value: 10^(-5+6) (µM/sec) (converted from M/sec) ranges 10^-7 M/sec to 10^-3 M/sec in Fig 3, based on varying levels of extracellular H202
else
...
if num_prx == 1
    ...
    % Solver options (to modify solver tolerances and step size)
